...
- [Phase 18-08]: PASSWORD_COMPLEXITY regex enforced on register only; login returns generic 'Invalid credentials' to avoid leaking requirements
- [Phase 18-08]: Lockout counter incremented for unknown usernames (prevents username enumeration via differential lockout timing)
- [Phase 18-08]: conversationSchema Zod validation in _getFromRedis: corrupt Redis data returns null (cache miss) for graceful degradation
