/** DM system prompt — the core personality and rules for the Dungeon Master AI. */
...
export function buildMultiplayerSystemPrompt(
  ...
  const roster = players
    .map((p) => `- ${p.displayName}: ${p.characterClass} (${p.pronouns || "They/Them"})`)
    .join("\n");
  const pronounInstructions = players
    .map((p) => `- ${p.displayName}: ${p.pronouns || "They/Them"}`)
    .join("\n");
  return (
    `${DM_SYSTEM_PROMPT}\n\n` +
    `## Multiplayer Party\n` +
...
/**
 * Build the system prompt with optional lore context and character class.
 */
export function buildSystemPrompt(options?: {
  ...
  if (options?.multiplayerPrompt) {
    systemPrompt = options.multiplayerPrompt;
  } else if (options?.characterClass) {
    ...
    systemPrompt = `${DM_SYSTEM_PROMPT}${pronounClause}`;
  }
